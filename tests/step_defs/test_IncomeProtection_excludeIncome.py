# coding=utf-8
"""Protection income linking and income excluded from scenario feature tests."""
import pytest
...
@scenario('../features/IncomeProtections_excludeIncome.feature', 'Verify the income and Death in service Protections linking then Exclude income from scenario')
def test_verify_the_income_and_death_in_service_protections_linking_then_exclude_income_from_scenario():
    """Verify the income and Death in service Protections linking then Exclude income from scenario."""
...
def verify_the_protections_for_any_breakages_protectiondescription(browser, ProtectionDescription):
    """Verify the Protections for any breakages <ProtectionDescription>."""
    page_protection = Protections(browser)
    page_protection.NavigatetoProtections()
...
def i_exclude_income_form_baseline_incomedescription(browser, IncomeDescription):
    """I exclude Income form Baseline <IncomeDescription>."""
    page_income = Income(browser)
    page_income.NavigatetoIncome()
    page_income.IncomeExclude(IncomeDescription)
